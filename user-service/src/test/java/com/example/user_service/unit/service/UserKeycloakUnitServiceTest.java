...
class UserKeycloakUnitServiceTest {
    ...
    void generateJwtToken_WithInvalidCredentials_ReturnsEmpty() {
        ...
        when(keycloakBuilder.build()).thenThrow(new NotAuthorizedException(""));

        // Act

        assertThrows(BadCredentialsException.class,
                () ->userKeycloakService.generateJwtToken(username, "wrongpassword"));
    }
    ...
    void generateJwtToken_WithValidCredentials_ReturnsJwt() {
        ...
        // Act
        Jwt result = userKeycloakService.generateJwtToken(username, "password");
    ...
    void getUser_WithEmptyUsername_ReturnsEmpty() {
        // Act
        Optional<UserRepresentation> result = userKeycloakService.getKeycloakUser("");
    ...
    void createUser_WhenCreationFails_ThrowsException() {
        ...
        assertThrows(RuntimeException.class, () ->
                userKeycloakService.createKeycloakUser(username, "John", "Doe", email, 123L, KeycloakRole.ROLE_CUSTOMER));
    ...
    void setUserPassword_WithValidData_ReturnsTrue() {
        ...
        // Act
        boolean result = userKeycloakService.setPasswordForUser(userId, password);
    ...
    void updateUserData_WithValidData_ReturnsTrue() {
        ...
        // Act
        boolean result = userKeycloakService.updateUserData(userId, "NewJohn", "NewDoe", "new@example.com" );
    ...
    void getUser_WithExistingUsername_ReturnsUser() {
        ...
        // Act
        Optional<UserRepresentation> result = userKeycloakService.getKeycloakUser(username);
    ...
    void userExists_WithExistingEmail_ReturnsEmailExists() {
        ...
        when(keycloakClientInstance.realm(realm)).thenReturn(realmResource);
        when(realmResource.users()).thenReturn(usersResource);
        when(usersResource.searchByEmail(email, true)).thenReturn(users);
