...
class SupportServiceUnitTest {
    ...
    // Используем те же константы, что и в сервисе (в верхнем регистре)
    private static final String SUPPORT_CHAT_CREATION_LIMITED = "SUPPORT_CHAT_CREATION_LIMITED";
    private static final String SUPPORT_MESSAGE_SENDING_LIMITED = "SUPPORT_MESSAGE_SENDING_LIMITED";

    @Test
    void createSupportChat_WhenNotLimited_CreatesChatSuccessfully() {
        // Given
        String redisKey = SUPPORT_CHAT_CREATION_LIMITED + ":" + USER_ID;
    ...
    void saveSupportMessage_WithValidData_SavesMessageAndUpdatesChat() {
        ...
        String redisKey = SUPPORT_MESSAGE_SENDING_LIMITED + ":" + CHAT_ID;
        when(redisService.get(redisKey)).thenReturn(Optional.empty());
    ...
    @Test
    void deleteSupportChat_WithValidChat_DeletesChat() {
        // Given
        SupportChat supportChat = new SupportChat();
        supportChat.setId(CHAT_ID);
        supportChat.setUserId(USER_ID);
        ...
        // Then
        verify(supportChatRepository).delete(supportChat);
    }
