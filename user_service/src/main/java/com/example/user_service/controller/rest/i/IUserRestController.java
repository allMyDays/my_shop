...
public interface IUserRestController {
    ...
    @Operation(
            summary = "Проверить изменение email",
            description = "Проверяет, изменился ли email пользователя"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Результат проверки",
                    content = @Content(schema = @Schema(type = "boolean", example = "false"))
            ),
            @ApiResponse(responseCode = "401", description = "Пользователь не аутентифицирован")
    })
    @SecurityRequirement(name = "JWT")
    ResponseEntity<Boolean> userEmailIsChanged(@Parameter(description = "Email для проверки", required = true, example = "user@example.com") @Email String email, Jwt jwt);
