...
public class CartServiceUnitTest {
    ...
    private final List<ProductIdAndQuantityDto> dtoList = List.of(new ProductIdAndQuantityDto(PRODUCT_ID, QUANTITY));
    @Test
    ...
    @Test
    void addItemsToCart_WhenCartIsFull_ReturnsZero() {
        // Given
        // Корзина полностью заполнена (лимит 4000)
        ...
        // When
        int added = cartService.addItemsToCart(USER_ID, dtoList);

        // Then
        assertEquals(0, added);
        verify(productGrpcClient, never()).productsExist(anyList());
        verify(cartItemRepository, never()).findByCartUserIdAndProductId(anyLong(), anyLong());
        verify(cartItemRepository, never()).save(any(CartItem.class));
    }

    @Test
    void addItemsToCart_WhenProductNotExists_ReturnsZeroAndDoesNotAdd() {
        // Given
        ...
        // Товар не существует — gRPC возвращает пустой список
        when(productGrpcClient.productsExist(List.of(PRODUCT_ID))).thenReturn(List.of());
        ...
        // Then
        assertEquals(0, added);
        verify(productGrpcClient).productsExist(List.of(PRODUCT_ID));
        verify(cartItemRepository, never()).findByCartUserIdAndProductId(anyLong(), anyLong());
        verify(cartItemRepository, never()).save(any(CartItem.class));
    }

    @Test
    void addItemsToCart_WhenProductExistsAndItemNotInCart_AddsNewItem() {
        // Given
        when(cartItemRepository.sumQuantityByUserId(USER_ID)).thenReturn(10);
        Cart cart = new Cart();
        cart.setUserId(USER_ID);
        ...
        when(productGrpcClient.productsExist(List.of(PRODUCT_ID))).thenReturn(List.of(PRODUCT_ID));
        ...
        // Then
        assertEquals(QUANTITY, added);
        verify(cartItemRepository).findByCartUserIdAndProductId(USER_ID, PRODUCT_ID);
        verify(cartItemRepository).save(any(CartItem.class));
    }

    @Test
    void addItemsToCart_WhenProductExistsAndItemInCart_UpdatesQuantity() {
        // Given
        when(cartItemRepository.sumQuantityByUserId(USER_ID)).thenReturn(10);
        Cart cart = new Cart();
        cart.setUserId(USER_ID);
    ...
    @Test
    void updateProductQuantity_DecreaseItem_ItemNotFound_ReturnsZero() {
        // Given
        when(cartItemRepository.findByCartUserIdAndProductId(USER_ID, PRODUCT_ID))
                .thenReturn(Optional.empty());
