...
import com.example.order_service.service.RoutingService;
...
class DeliveryInfoServiceUnitTest {
    ...
    void setDeliveryInfo_WithInvalidAddress_ThrowsAddressNotCorrectException() {
        ...
        when(daDataClient.validateAndGetAddressInfo(invalidAddress)).thenReturn(Optional.empty());

        // When & Then
        assertThrows(AddressNotCorrectException.class,
                () -> deliveryInfoService.setDeliveryInfo(USER_ID, invalidAddress));

        verify(daDataClient).validateAndGetAddressInfo(invalidAddress);
        verify(deliveryRepository, never()).save(any());
    }

    @Test
    void setDeliveryInfo_WithSameExistingAddress_DoesNothing() {
        // Given
        String sameAddress = "г. Москва, ул. Ленина, 1";
        UserAddressDto userAddressDto = new UserAddressDto();
        userAddressDto.setFullAddress(sameAddress);

        DeliveryInfo existingInfo = new DeliveryInfo();
        existingInfo.setUserId(USER_ID);
        existingInfo.setUserAddress(sameAddress);

        when(daDataClient.validateAndGetAddressInfo(sameAddress)).thenReturn(Optional.of(userAddressDto));
        when(deliveryRepository.findById(USER_ID)).thenReturn(Optional.of(existingInfo));

        // When
        deliveryInfoService.setDeliveryInfo(USER_ID, sameAddress);

        // Then
        verify(daDataClient).validateAndGetAddressInfo(sameAddress);
        verify(deliveryRepository).findById(USER_ID);
        verify(deliveryRepository, never()).save(any());
    }
    ...
    void setDeliveryInfo_WithNewAddress_CreatesNewDeliveryInfo() {
        ...
        when(daDataClient.validateAndGetAddressInfo(newAddress)).thenReturn(Optional.of(userAddressDto));
        when(deliveryRepository.findById(USER_ID)).thenReturn(Optional.empty());
        when(routingService.testRoute(eq(55.7558), eq(37.6173), eq(55.649108), eq(37.528433)))
        ...
        // Then
        verify(daDataClient).validateAndGetAddressInfo(newAddress);
        verify(routingService).testRoute(55.7558, 37.6173, 55.649108, 37.528433);
