...
class ReviewServiceUnitTest {
    ...
    void edit_WithValidData_UpdatesReview() {
        ...
        oldReview.setAdvantages(null);
        oldReview.setDisAdvantages(null);
        oldReview.setComment(null);

        when(reviewRepository.findById(REVIEW_ID)).thenReturn(Optional.of(oldReview));

        // When
        reviewService.edit(REVIEW_ID, USER_ID, 5, false, UsagePeriod.LESS_THAN_MONTH,
                null, null, "Updated comment", null, null);
        ...
        verify(reviewRepository).save(argThat(review ->
                review.getEditingQuantity() == 1 &&
                        ...
                        "Updated comment".equals(review.getComment()) &&
                        review.getAdvantages() == null &&
                        review.getDisAdvantages() == null
        ));
        verify(mediaKafkaClient, never()).deleteMedia(anyList());
        verify(mediaGrpcClient, never()).uploadPhotos(anyList(), any());
    }
    ...
    void edit_WithImagesToDelete_DeletesImages() {
        ...
        oldReview.setDateOfCreation(LocalDateTime.now().minusDays(1));
        oldReview.setEditingQuantity(0);
        oldReview.setPhotoFileNames(new ArrayList<>(List.of("image1.jpg", "image2.jpg", "image3.jpg")));
        ...
        // When
        reviewService.edit(REVIEW_ID, USER_ID, 5, false, UsagePeriod.LESS_THAN_MONTH,
                null, null, null, null, imagesToDelete);
    ...
    void edit_WithNoChanges_ThrowsNoChangesInEditingReviewException() {
        ...
        assertThrows(NoChangesInEditingReviewException.class,
                () -> reviewService.edit(REVIEW_ID, USER_ID, 4, false, UsagePeriod.LESS_THAN_MONTH,
                        "Good quality", "Expensive", "Nice product", null, null));
