...
public class MinioService {
    ...
    public boolean deleteFile(@NonNull String fileName){
        ...
        try{ s3Client.deleteObject(DeleteObjectRequest
           ...
           return true;
            ...
            log.warn("error while deleting file with fileName: {}",fileName);
           return false;
       }
    }
    ...
    public int deleteMultipleFiles(@NonNull List<String> keys) {
        if(keys.isEmpty()||keys.size()>1000){
            throw new RuntimeException("Размер массива ключей либо пуст, либо содержит больше чем 1000 элементов.");
        }
        try {
    ...
    public List<String> uploadFiles(@NonNull List<PhotoDataDTO> files, @NonNull BucketEnum bucket) {
        ...
        for (PhotoDataDTO file : files) {
            ...
            try {
                ...
                log.warn("error while uploading file. Bucket: {}, ContentType: {}, Key: {}",bucket.name(),file.getContentType(),newFileName);
            }
        } return newFileNames;
    }
